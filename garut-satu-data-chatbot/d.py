...
def handle_dataset_search(query: str):
    """
    Fungsi untuk mencari dataset berdasarkan query pengguna.
    Jika tidak ada hasil di bawah threshold, ambil kandidat terdekat dan tetap lakukan analisis.
    """
    log.info("[DATA_AGENT] Mencari dataset untuk query: %s", query)
    try:
        # Ambil hasil embedding search
        candidates = search_dataset_embeddings(query)
        if not candidates:
            log.warning("[DATA_AGENT] Tidak ada hasil embedding untuk query: %s", query)
            return {
                "status": "error",
                "query": query,
        ...
        in_threshold = [c for c in candidates if c.get("_distance") is not None and c.get("_distance") < DISTANCE_THRESHOLD]

        # Jika tidak ada kandidat dalam threshold, ambil kandidat terdekat
        if not in_threshold:
            best_candidate = candidates[0]
            log.warning(
                "[DATA_AGENT] Tidak ada kandidat < %.2f, fallback ke dataset terdekat: %s (dist=%.3f)",
                DISTANCE_THRESHOLD,
                best_candidate.get("title"),
                best_candidate.get("_distance"),
            )
            in_threshold = [best_candidate]

        # Ambil dataset pertama yang relevan
        chosen = in_threshold[0]
        title = chosen.get("title", "Dataset tanpa judul")
        url = chosen.get("url")
        download_url = chosen.get("download_url")

        log.info("[DATA_AGENT] Dataset terpilih: %s", title)
        log.debug("[DATA_AGENT] URL dataset: %s", url)
        log.debug("[DATA_AGENT] Download URL: %s", download_url)

        # Load data dari dataset
        df = load_full_dataframe_from_url(download_url)
        if df is None or df.empty:
            log.warning("[DATA_AGENT] Dataset kosong untuk %s", title)
            return {
                "status": "error",
                "query": query,
        ...
        # Analisis isi data dengan LLM
        try:
            analysis = analyze_data_with_llm(query, df)
        except Exception as e:
            log.exception("[DATA_AGENT] Gagal menjalankan analisis LLM: %s", e)
            analysis = "Gagal melakukan analisis otomatis terhadap isi dataset."

        # Buat pratinjau tabel (maks 5 baris)
        ...
        log.exception("[DATA_AGENT] Terjadi error saat handle_dataset_search: %s", e)
        return {
            "status": "error",
            "query": query,
            "error_message": f"Terjadi error internal saat mencari dataset: {str(e)}"
        }
...
def handle_general_question(query: str) -> dict:
    if site_guide_collection is None:
        return {"reply": "Error: Database panduan situs tidak dapat diakses."}
    try:
        emb = embedding_model.encode([query]).tolist()
        res = site_guide_collection.query(query_embeddings=emb, n_results=1, include=["documents", "distances"])
        docs = res.get("documents", [[]])[0]
        if not docs:
            return {"reply": "Maaf, saya tidak dapat menemukan informasi yang relevan dengan pertanyaan umum Anda."}
        context = docs[0]
        prompt = (
            "Anda adalah asisten AI 'Satu Data Garut'. Jawab singkat berdasarkan konteks berikut (fokus hanya Kabupaten Garut).\n\n"
            f"{context}\n\nPERTANYAAN: {query}\nJAWABAN:"
        )
        # Use safe wrapper
...
def handle_chat():
    """Endpoint utama. Input JSON: { "query": "<text>" }.
       Output JSON: { "reply": "<text>", ... } (consistent)
    """
    try:
        ...
        if intent == "dataset_sector_search":
            try:
                # user types exactly: "Tampilkan dataset sektor <X>"
        ...
        if intent == "run_data_agent":
            ...
            if successful:
                # Prepare final reply: combine analysis answers and dataset metadata blocks
                pieces = []
                for r in successful:
                    # use keys available from handle_dataset_search
                    pieces.append(f"**{r.get('dataset_title')}**\n{r.get('ai_analysis')}\n")
                    pieces.append(f"Pratinjau data: (lihat field 'data_preview' untuk table)\n")
                    pieces.append(f"Sumber: {r.get('dataset_url')}\n---\n")
                final_reply = "\n".join(pieces).strip()
                cache_set(user_query, final_reply)
                return jsonify({"reply": final_reply, "results": successful}), 200
            else:
                # no successful; return combined errors/hints
        ...
        # Log full exception server-side, but return friendly message to client with 200 OK
        log.exception("[HANDLE_CHAT] Unhandled exception: %s", e)
        # keep response stable for frontend (avoid 5xx)
...
if __name__ == "__main__":
    log.info("Starting chatbot on http://0.0.0.0:5000")
